...
$(document).ready(function() {
	...
	$(document).bind("forminvalid.zf.abide", function(ev,frm) {
		...
		if (debugMode) console.log("Form id "+ev.target.id+" is invalid");
		if (debugMode) console.log(invalid_fields);
	...
	.bind("formvalid.zf.abide", function(ev,frm) {
		...
		if (debugMode) console.log("Form id "+frm.attr('id')+" is valid");
	...
	_arr = dzOptions['acceptedFiles'].split(',');
...
function normalizeFilename(str) {
	...
	if (debugMode) console.log('FUNC: normalizeFilename ' + str);
            ...
            function videoRendered(videoEl) {
                ...
                frameGrab.grab(imgEl, 1, 160).then(
                    ...
                    function frameFailedToGrab(reason) {
                        if (debugMode) console.log("FUNC: createVideoThumb - ERROR: Can't grab the video frame from file: " +
                            file.name + ". Reason: " + reason);
                    }
                );
            },
            function videoFailedToRender(reason) {
                if (debugMode) console.log("FUNC: createVideoThumb - ERROR: Can't convert the file to a video element: " +
                    file.name + ". Reason: " + reason);
            }
        );
    }
...
Dropzone.options.DwDropzone = {
	...
	init: function() {
		if (debugMode) console.log('EVENT: init');
		...
		submitButton.addEventListener("click", function(e) {
			if (debugMode) console.log('submit ...');
			...
			if (isValidForm && (meDropzone.files != "")) {
				if (debugMode) console.log('meDropzone.processQueue() --> files: ' + meDropzone.files);
				// Upload files and submit form
				meDropzone.processQueue();
			} else {
				showMessage(msgFormInvalid, 'alert');
				if (debugMode) console.log(isValidForm);
			}
		...
		resetButton.addEventListener("click", function(e) {
			if (debugMode) console.log('EVENT: resetButton : Click');
		...
		this.on("addedfile", function(file) {
			if (debugMode) console.log('EVENT: addedfile -> file: ' +  JSON.stringify(file));
			...
			if (this.files.length) {
				...
				for (_i = 0, _len = this.files.length; _i < _len - 1; _i++) // -1 to exclude current file
				{
					if(preventDuplicates && this.files[_i].name === file.name && this.files[_i].size === file.size && this.files[_i].lastModifiedDate.toString() === file.lastModifiedDate.toString()) {
						showMessage(msgDuplicatesNotAllowed, 'alert');
		...
		this.on("totaluploadprogress", function (uploadProgress, totalBytes , totalBytesSent) {
			//if (debugMode) console.log("EVENT: totaluploadprogress -> uploadProgress: ", uploadProgress);
		...
		this.on("uploadprogress", function (file, progress, bytesSent) {
			//if (debugMode) console.log("EVENT: uploadprogress -> file.name: " + file.name + ", progress: ", progress);
		...
		// 	if (debugMode) console.log('queuecomplete ', progress);
		// 	progressTotal.style.height = "0";
		// });

		// Event: maxfilesexceeded
		this.on("maxfilesexceeded", function(file){
			if (debugMode) console.log('EVENT: maxfilesexceeded');
		...
		this.on("sendingmultiple", function() {
			if (debugMode) console.log('EVENT: sendingmultiple');
			// Gets triggered when the form is actually being sent.
			// Hide the success button or the complete form.
		});

		this.on("successmultiple", function(files, response) {
			if (debugMode) console.log('EVENT: successmultiple');
			// Gets triggered when the files have successfully been sent.
			// Redirect user or notify of success.
		});

		this.on("success", function(files, response) {
			if (debugMode) console.log('EVENT: success');
			showMessage(msgTransferSuccess, 'success');
		});

		this.on("canceled", function(file) {
			if (debugMode) console.log('EVENT: canceled -->' + file);
		...
		this.on("error", function(files, response) {
			if (debugMode) console.log('EVENT: error');
			showMessage(response, 'alert');
		});

		this.on("errormultiple", function(files, response) {
			if (debugMode) console.log('EVENT: errormultiple');
			// Gets triggered when there was an error sending the files.
			// Maybe show form again, and notify user of error
		});

		this.on("dragstart", function(event) {
			if (debugMode) console.log("EVENT: dragstart: " + event);
			// Gets triggered when the files have successfully been sent.
			// Redirect user or notify of success.
		});
	...
	// 	if (debugMode) console.log('EVENT: sending');
	// 	if (debugMode) console.log('EVENT: sending -> fromData: ' + JSON.stringify(formData));
	// 	if (debugMode) console.log('EVENT: sending -> xhr: ' + JSON.stringify(xhr));
	// 	// if (debugMode) console.log('EVENT: sending -> file: ' + file);
	...
	chunksUploaded: function(file, done) {
		if (debugMode) console.log("EVENT: chunksUploaded --> " + file);
		done();
	},

	accept: function(file, done) {
		if (debugMode) console.log("EVENT: Accept: uploaded");
		done();
	},
	...
	thumbnail: function(file, dataUrl) {
		...
		if (debugMode) console.log('EVENT: thumbnail');
	}
