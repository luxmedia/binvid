<?php
   ...
   if (!empty($_FILES)){
    ...
    function resumableUpload($tmp_file_path, $filename){
		...
		print "Filename: " .$_POST["newFileName"]; 
		print "<br> Desti: ". $_POST["newDestination"]; 
    ...
    function createFileFromChunks($file_chunks_folder, $fileName, $newFileName , $extension, $total_size, $total_chunks,
		...
		$avidPath = "functions/uploads/completed/AVID/";
		$vpmsPath = "functions/uploads/completed/VPMS/";
		$medialoopsterPath = "functions/uploads/completed/MediaLoopster/";
		
		//Verteilung SubSysteme FTP 
		if(strtolower($newDestination) == "avid"){
			$rel_path = $avidPath;
		}
		else if(strtolower($newDestination) == "vpms"){
			$rel_path = $vpmsPath;
		}
		else if(strtolower($newDestination) == "medialoopster"){
			$rel_path = $medialoopsterPath;
		}
